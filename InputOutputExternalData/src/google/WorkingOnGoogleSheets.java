...
public class WorkingOnGoogleSheets {
...
//    InputStream inputStream = WorkingOnGoogleSheets.class.getResourceAsStream("/credentials.json");
//	  Use this if your project is created using maven and you have a resource folder

		InputStreamReader credincialLocation = new InputStreamReader(is);
...
//  https://docs.google.com/spreadsheets/d/1MKR8pXipURJGeb9ZXSWVVO0JIJPK-QGQbR73DnfpYbY/edit#gid=0
	...
	public static void SingleRangeWrite() throws IOException, GeneralSecurityException {
		ValueRange range = new ValueRange().setValues(Arrays.asList(Arrays.asList("Expenses January"),
				Arrays.asList("books", "30"), Arrays.asList("pens", "10"), Arrays.asList("Expenses February"),
				Arrays.asList("cloths", "20"), Arrays.asList("shoes", "5")));
		UpdateValuesResponse result = sheetService.spreadsheets().values().update(spreadSheetID, "A1", range)
...
//  --------------------------MiltiRange Writes------------------------------
	...
	public static void BatchUpdate() throws IOException {
		...
		data.add(new ValueRange().setRange("D1").setValues(Arrays.asList(Arrays.asList("Jan Total", "=B2+B3"))));
		data.add(new ValueRange().setRange("D4").setValues(Arrays.asList(Arrays.asList("Feb Total", "=B5+B6"))));
...
//  ---------------------------------Append Data -----------------------------
	...
	public static void AppendingData() throws IOException {
		...
		AppendValuesResponse appendResult = sheetService.spreadsheets().values().append(spreadSheetID, "A1", appendBody)
...
//          System.out.println(total);
	...
	// -----------------------------------------Reading value from Sheet within
	// multiple range---------
...
//      Rename SpreadSheet
		requests.add(new Request().setUpdateSpreadsheetProperties(new UpdateSpreadsheetPropertiesRequest()
				.setProperties(new SpreadsheetProperties().setTitle("Selenim Expenses")).setFields("title")));
//      Add new Sheet to spreadsheet
		requests.add(new Request().setAddSheet(
				new AddSheetRequest().setProperties(new SheetProperties().setSheetId(1).setTitle("expense"))));

//      Rename sheet

		requests.add(new Request().setUpdateSheetProperties(new UpdateSheetPropertiesRequest()
				.setProperties(new SheetProperties().setSheetId(1).setTitle("Hello")).setFields("title")));

//      Rename Field
		requests.add(new Request().setFindReplace(
				new FindReplaceRequest().setFind("Total").setReplacement("New Total").setAllSheets(true)));

//      copy paste
		CopyPasteRequest copyPasteRequest = new CopyPasteRequest()
				.setSource(new GridRange().setSheetId(0).setStartColumnIndex(0).setEndColumnIndex(5).setStartRowIndex(0)
						.setEndRowIndex(4))
				.setDestination(new GridRange().setSheetId(1).setStartColumnIndex(4).setEndColumnIndex(9)
